...
import { Search, Filter, MoreVertical, ArrowLeft, Loader2 } from "lucide-react";
...
const DoctorsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDoctors = async () => {
      try {
        setIsLoading(true);
        const response = await api.get(Constants.urlEndPoints.GET_DOCTORS, {
          signal: controller.signal
        });

        if (response.data.success) {
          const docsData = Array.isArray(response.data.data) ? response.data.data : [];

          const formattedDoctors = docsData.map((doc) => {
            // ✅ Combine both degrees
            const degrees = [doc.underGraduationDegree, doc.postGraduationDegree].filter(Boolean);
            const qualificationString = degrees.length > 0 ? degrees.join(", ") : "MBBS";

            return {
              id: doc._id,
              name: doc.name,
              qualification: qualificationString, // Shows "MBBS, MD Pediatrics"
              specialty: doc.primarySpecialty || "General",
  ...
  const handleToggle = async (id, currentStatus) => {
    ...
    try {
      await api.patch(`${Constants.urlEndPoints.GET_DOCTORS}/${id}/status`, {
      ...
      setDoctors((prev) =>
        prev.map((doc) => (doc.id === id ? { ...doc, available: currentStatus } : doc))
      );
    } finally {
      setUpdatingId(null);
    }
  ...
  const handleAddDoctor = () => {
    router.push("/hospitaldashboard/add-doctors");
  };
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {doc.days.map((dayObj, index) => {
                        ...
                        return (
                          <span 
                            key={index} 
                            className={`text-[10px] sm:text-xs px-2.5 py-1 rounded-md font-medium transition-colors border ${
                              dayObj.active 
                ...
                <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start w-full md:w-auto gap-4 md:gap-2 mt-2 md:mt-0 pt-3 md:pt-0 border-t md:border-t-0 border-gray-50">
                  ...
                  <div className="flex items-center gap-3 bg-gray-50 md:bg-transparent p-2 md:p-0 rounded-lg w-full md:w-auto justify-between md:justify-end">
                    ...
                    <ToggleSwitch 
                      ...
                      onChange={() => handleToggle(doc.id, doc.available)} 
                    />
                  </div>
                </div>
              </div>
